...
def kitti_eval(gt_annos,
            ...
            for idx in range(3):
                ...
                if mAP11_bbox is not None:
                    ret_dict[f'{prefix}_2D_AP11_{postfix}'] =\
                        mAP11_bbox[j, idx, i]
                # NOTE:logger里加上aos
                if mAP11_aos is not None:
                    ret_dict[f'{prefix}_AOS_AP11_{postfix}'] =\
                        mAP11_aos[j, idx, i]
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            ...
            # NOTE:加上aos
            if mAP11_aos is not None:
                ret_dict[f'KITTI/Overall_AOS_AP11_{postfix}'] =\
                    mAP11_aos[idx, 0]
            ...
            for idx in range(3):
                ...
                if mAP40_bbox is not None:
                    ret_dict[f'{prefix}_2D_AP40_{postfix}'] =\
                        mAP40_bbox[j, idx, i]
                # NOTE:加上aos
                if mAP40_aos is not None:
                    ret_dict[f'{prefix}_AOS_AP40_{postfix}'] =\
                        mAP40_aos[j, idx, i]  
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            ...
            # NOTE:加上aos
            if mAP40_aos is not None:
                ret_dict[f'KITTI/Overall_AOS_AP40_{postfix}'] =\
                    mAP40_aos[idx, 0]
