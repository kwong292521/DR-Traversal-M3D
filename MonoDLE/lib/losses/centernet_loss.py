...
def compute_loss(input, target, cur_epoch=None):
    stats_dict = {}

    #============= Baseline Loss =============#
    if cfg['model']['type'] in ['centernet3d@rgb']:
        base_loss, base_loss_term = compute_centernet3d_loss(input, target, modality='rgb')
        stats_dict.update(base_loss_term)
...
def compute_centernet3d_loss(input, target, modality='rgb'):
    stats_dict = {}
    ...
    # handle nan
    [seg_loss, offset2d_loss, size2d_loss, offset3d_loss, depth_loss, size3d_loss, heading_loss] = list(map(lambda x:judge_nan(x),
        [seg_loss, offset2d_loss, size2d_loss, offset3d_loss, depth_loss, size3d_loss, heading_loss]))

    # statistics
    stats_dict[f'{modality}_seg'] = seg_loss.item()
    stats_dict[f'{modality}_offset2d'] = offset2d_loss.item()
    stats_dict[f'{modality}_size2d'] = size2d_loss.item()
    stats_dict[f'{modality}_offset3d'] = offset3d_loss.item()
    stats_dict[f'{modality}_depth'] = depth_loss.item()
    stats_dict[f'{modality}_size3d'] = size3d_loss.item()
    stats_dict[f'{modality}_heading'] = heading_loss.item()

    total_loss = seg_loss + offset2d_loss + size2d_loss + offset3d_loss + \
                 depth_loss + size3d_loss + heading_loss
    
    stats_dict[f'{modality}_total_loss'] = total_loss.item()
...
def compute_depth_loss(input, target):
    ...
    # depth clamp for stable training at early stage
    # depth_input = torch.clamp(depth_input, min=2, max=65)
...
def compute_offset3d_loss(input, target):
    offset3d_input = extract_input_from_tensor(input['offset_3d'], target['indices'], target['mask_2d'])
    offset3d_target = extract_target_from_tensor(target['offset_3d'], target['mask_2d'])
...
def compute_size3d_loss(input, target):
    size3d_input = extract_input_from_tensor(input['size_3d'], target['indices'], target['mask_2d'])
    size3d_target = extract_target_from_tensor(target['size_3d'], target['mask_2d'])
...
def compute_heading_loss(input, target):
    ...
    heading_input_cls, heading_target_cls = heading_input_cls[mask.to(torch.bool)], heading_target_cls[mask.to(torch.bool)]
    if mask.sum() > 0:
    ...
    heading_input_res, heading_target_res = heading_input_res[mask.to(torch.bool)], heading_target_res[mask.to(torch.bool)]
    cls_onehot = torch.zeros(heading_target_cls.shape[0], 12).to(device).scatter_(dim=1, index=heading_target_cls.view(-1, 1), value=1)
...
def extract_target_from_tensor(target, mask):
    return target[mask.to(torch.bool)]
